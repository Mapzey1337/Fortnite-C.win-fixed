...
namespace Settings {
	namespace Selection {
		bool bMouseAimbotEnabled = true,Prediction = true, bLineESPEnabled = true, bDistanceESP = true, bDrawFOV = false, bVisualName = true, bBox = false, outline = true, skel = true;
		int bBoxMode = false, showhead = false, distance, MaxSkeletonDrawDistance = 500, hitbox = false, EspDistance = 500, AimbotDistance = 150;
...
{
	uint64_t bonearray = read<uint64_t>(mesh + 0x4b8);
	//printf("bonearray :0x%llX\n spoily", bonearray);
	if (!bonearray) bonearray = read<uint64_t>(mesh + 0x4b8);
